...
class NotificationHelper
{
    public static function web_panel_notification($key, $data_type, $data_id = null)
    {
        ...
        if (Auth::user()->hasRole('property_admin')) {
            if ($data_type == 'property_id') {
                ...
                $users = User::where('id', $user_assign->pluck('user_id')->toArray())->get();
        ...
        if ($notification_message !== null) {
            if (Auth::user()->hasRole('property_admin')) {
                NotificationHelper::notification_create($key, $notification_message, $data, $data_type, ['property_id', 'employee_id', 'sale_id'], $users, $user);
            } elseif (Auth::user()->roles[0]->name == 'property_manager') {
                NotificationHelper::notification_create($key, $notification_message, $data, $data_type, ['property_id', 'employee_id', 'sale_id'], $users, $user);
            } elseif (Auth::user()->roles[0]->name == 'sale_manager') {
                NotificationHelper::notification_create($key, $notification_message, $data, $data_type, ['property_id', 'employee_id', 'sale_id'], $users, $user);
            } elseif (Auth::user()->hasRole('sale_person')) {
                NotificationHelper::notification_create($key, $notification_message, $data, $data_type, ['sale_id'], $users, $user);
            } else {
    ...
    private static function notification_create($key, $notification_message, $data, $data_type, $data_type_arr, $users = null, $user = null)
    {
