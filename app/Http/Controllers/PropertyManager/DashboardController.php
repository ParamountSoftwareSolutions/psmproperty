...
//dd($data);
    ...
    {
        //$data = [];
        $building = (new Helpers)->building_detail_single($id);

        $sales = BuildingSale::with('customer')->where(['property_admin_id' => $building->user_id, 'building_id' => $building->id])->where(['order_type' => 'sale'])
        ->get();
        $leads = BuildingSale::where(['property_admin_id' => $building->user_id, 'building_id' => $building->id, 'order_type' => 'lead'])->get();
        $sale = BuildingSale::where(['property_admin_id' => $building->user_id, 'building_id' => $building->id, 'order_type' => 'sale'])->get();
