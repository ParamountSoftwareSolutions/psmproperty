...
class NotificationController extends Controller
{
    public function index()
    {
        if (Auth::user()->hasRole('property_admin')) {
            $notification = Notification::where('receiver_id', Auth::id())->latest()->get();
    ...
    public function latest()
    {
        if (Auth::user()->roles[0]->name == 'property_admin'){
            $notification = Notification::where('receiver_id', Auth::id())->where('read_at', null)->get();
    ...
    {
        if (Auth::user()->roles[0]->name == 'property_admin'){
            $notification = Notification::where('receiver_id', Auth::id())->where('read_at', null)->update(['read_at' =>
                Carbon::now()]);
        } elseif(Auth::user()->roles[0]->name == 'property_manager') {
            $notification = Notification::where('receiver_id', Auth::id())->where('read_at', null)->update(['read_at' => Carbon::now()]);
        } elseif(Auth::user()->roles[0]->name == 'sale_manager') {
            $notification = Notification::where('receiver_id', Auth::id())->where('read_at', null)->update(['read_at' => Carbon::now()]);
        } elseif(Auth::user()->roles[0]->name == 'sale_person') {
            $notification = Notification::where('receiver_id', Auth::id())->where('read_at', null)->update(['read_at' => Carbon::now()]);
    ...
    public function mark_single_read_notification($panel, $id)
    {
        if (Auth::user()->roles[0]->name == 'property_admin'){
            $notification = Notification::where('receiver_id', Auth::id())->latest()->get();
            $update = Notification::where('receiver_id', Auth::id())->where('read_at', null)->where('id', $id)->update(['read_at' =>
                Carbon::now
            ()]);
        } elseif(Auth::user()->roles[0]->name == 'property_manager') {
            $notification = Notification::where('receiver_id', Auth::id())->latest()->get();
            ...
            $notification = Notification::where('receiver_id', Auth::id())->latest()->get();
            $update = Notification::where('receiver_id', Auth::id())->where('read_at', null)->where('id', $id)->update(['read_at' => Carbon::now()]);
        } elseif(Auth::user()->roles[0]->name == 'sale_person') {
            $notification = Notification::where('receiver_id', Auth::id())->latest()->get();
            $update = Notification::where('receiver_id', Auth::id())->where('read_at', null)->where('id', $id)->update(['read_at' => Carbon::now()]);
        } else {
        ...
        if (request()->ajax() == true){
            ...
            return redirect()->back()->with($this->message('Notification mark as read successfully!', 'success'));
        }
