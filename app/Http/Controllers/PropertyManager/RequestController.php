    ...
    public function update(Request $req, $type, $id)
    {
        ...
        if ($type == 'transfer') {
            ...
            if ($req->status == 'accept') {
                (new NotificationHelper)->send_notification_single_user('transfer_accept', $request->transfer_to, $data);
                (new NotificationHelper)->web_panel_notification('transfer_accept');
            } elseif ($req->status == 'rejected') {
                (new NotificationHelper)->send_notification_single_user('transfer_rejected', $request->transfer_to, $data);
                (new NotificationHelper)->web_panel_notification('transfer_rejected');
            }
        } elseif ($type == 'possession') {
            $data = ['title' => $req->title, 'description' => $req->description];
            if ($req->status == 'accept') {
                (new NotificationHelper)->send_notification_single_user('possession_accept', $request->transfer_to, $data);
                (new NotificationHelper)->web_panel_notification('possession_accept');
            } elseif ($req->status == 'rejected') {
                (new NotificationHelper)->send_notification_single_user('possession_rejected', $request->transfer_to, $data);
                (new NotificationHelper)->web_panel_notification('possession_rejected');
            }
        } elseif ($type == 'file') {
            $data = ['title' => $req->title, 'description' => $req->description];
            if ($req->status == 'accept') {
                (new NotificationHelper)->send_notification_single_user('file_accept', $request->transfer_to, $data);
                (new NotificationHelper)->web_panel_notification('file_accept');
            } elseif ($req->status == 'rejected') {
                (new NotificationHelper)->send_notification_single_user('file_rejected', $request->transfer_to, $data);
                (new NotificationHelper)->web_panel_notification('file_rejected');
            }
        } elseif ($type == 'reserve') {
            $data = ['title' => $req->title, 'description' => $req->description];
            if ($req->status == 'accept') {
                (new NotificationHelper)->send_notification_single_user('reserve_accept', $request->transfer_to, $data);
                (new NotificationHelper)->web_panel_notification('reserve_accept');
            } elseif ($req->status == 'rejected') {
                (new NotificationHelper)->send_notification_single_user('reserve_rejected', $request->transfer_to, $data);
