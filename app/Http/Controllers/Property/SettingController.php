...
class SettingController extends Controller
{
    public function push_notification()
    {
        ...
        if (BuildingSetting::where(['user_id' => Auth::id(), 'user_type' => Auth::user()->roles[0]->name, 'key' => 'sale_mature'])->first() == false) {
            BuildingSetting::insert([
                'user_id' => Auth::id(),
                'user_type' => Auth::user()->roles[0]->name,
                'key' => 'sale_mature',
    ...
    public function update_push_notification(Request $request)
    {
        ...
        BuildingSetting::updateOrInsert(['user_id' => Auth::id(), 'user_type' => Auth::user()->roles[0]->name, 'key' => 'sale_mature'], [
            'value' => json_encode([
                'status' => $request['sale_mature_status'] == 'on' ? 1 : 0,
                'message' => $request['sale_mature_message'],
            ]),
        ]);
