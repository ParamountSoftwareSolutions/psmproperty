...
@php
    ...
    if($meeting !== null){
        $time = \Carbon\Carbon::parse(json_decode($meeting->data)->date)->format('h:i A');
        $id = $meeting->id;
        $read = json_decode($meeting->data)->is_read;
    } else {
        $id = '';
        $time = '';
        $read = '';
...
<script>
    ...
    let time = "{{ $time }}";
    let read = "{{ $read }}";
    if(!id == false && !time == false && read ==0){
        meetingAlert(time,id);
...
<script>
    $(document).ready(function () {
        ...
        setInterval(function () {
            ...
            $.ajax({
                url: "{{ url('property-manager/notification/latest') }}",
                type: "GET",
                dataType: "json",
                success: function (data) {
                    console.log(data)
