    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content p-5 pt-0">
            <div class="intro-y box py-10 sm:py-20">
                <form id="agent-form" action="{{url('agent/societies/sale')}}" method="POST" enctype="multipart/form-data" >
                    @csrf
                    <div id="steps-1" class="steps px-5 sm:px-20 mt-10 pt-10 border-t border-gray-200 dark:border-dark-5">
                        <div class="grid grid-cols-12 gap-4 gap-y-5 mt-5">
                            ...
                            <input type="hidden"  id="sales_cat_id" />
                            <input type="hidden" id="plotSize" name="plot_size"/>
                            <input type="hidden" name="society_id" id="society_id"/>
                            <input type="hidden" name="sale_category_id" id="society_cat_id"/>
                            ...
                            <div class="intro-y col-span-6 sm:col-span-6">
                                ...
                                <input id="file_number" name="file_number" type="text" class="form-control" onchange="checkValue()" placeholder="file_number">
...
<script type="text/javascript">
    ...
    var checkValue = function(){
        ...
        var minNumber = extractNumber(min); //3
        var maxNumber = extractNumber(max); //3

        //get aplhabet from min

        var alphabets = extractAlphabet(min); //rm


        var currentFile = $('#file_number').val();

        var currentFileNumber = extractNumber(currentFile);
        var currentFileApha = extractAlphabet(currentFile);

        if(currentFileApha.localeCompare(alphabets) == 0 && currentFileNumber >= minNumber && currentFileNumber <= maxNumber){
            $.ajax({
                url: "{{url('agent/societies/plot/status')}}",
                data: {reg_num: currentFile},
                type: 'GET',
                success:function(response){
                    if(response.result == "false"){
                        ...
                        getCategoryAdditionalData('{{url('agent/societies/category/get-additional-data')}}');

                    }else{
                        // already booked
                        $('#file_number').addClass('border-danger');
                        $('#btn-1').attr('disabled', 'disabled');
                        $('#msg-text').text("File Number Already Booked For Someone else");
                    }
                },
                error: function(error){
                }
            ...
            $('#msg-text').text("Please recheck your provided file number");
            $('#file_number').addClass('border-danger');
            $('#btn-1').attr('disabled', 'disabled');
        }
    ...
    function extractAlphabet(string){
