            ...
            <div class="section-body">
                <div class="row">
                    <div class="col-12 col-md-12 col-lg-12">
                        <div class="card">
                            <form method="post" action="{{ route('property_manager.privacyPolicy.update', $privacyPolicy->id) }}">
                                @csrf
                                @method('put')
                                        ...
                                        <div class="form-group col-md-4">
                                            ...
                                            <select class="form-control select2" multiple="" name="building_id[]">
                                                ...
                                                @foreach($building as $data)
                                                    <option value="{{ $data->id }}" @if(in_array($data->id, json_decode($privacyPolicy->building_id))) selected @endif>{{ $data->name }}</option>
                                                @endforeach
                                            </select>
                                            @error('building_id')
