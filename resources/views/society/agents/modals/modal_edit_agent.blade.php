    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content p-5 pt-0">
            <div class="intro-y box py-10 sm:py-20">
                <form id="agent-form" action="{{url('society/agent/update')}}" method="POST" enctype="multipart/form-data" >
                    @csrf
        ...
        $.ajax({
            ...
            success:function(response){
               ...
               response.data.agents.forEach(function (item, index) {
                   html += '<li onclick=getAgentDetails("'+ item.id + '","'+ item.name + '","'+ item.business_address+ '","'+ item.contact_number+ '")><a href="#">'+item.name+'</a></li>';
               });
               html += "</ul>";
